...
class MysqliDb
{
     ...
     */
    public function get($tableName, /*ajout de ce parametre*/$selects=null, $numRows = null)
    {   
        ...
        if(isset($selects)){
            for ($i=0; $i < count($selects); $i++) { 
               $this->_query .= $selects[$i];
               if($i+1!=count($selects)){
                     $this->_query .= ", ";
               }else{
                    $this->_query .= " ";
               }
        ...
        // fin modification du code originale
        $stmt = $this->_buildQuery($numRows);
        $stmt->execute();
        $this->reset();
